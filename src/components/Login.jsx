...
const Login = () => {
  ...
  useEffect(() => {
    // ✅ Only show logout modal if user navigates directly to /login
    if (user && location.pathname === "/login") {
      setShowLogoutModal(true);
    }
  }, [user, location.pathname]); // Runs only on direct navigation
      ...
      <div className="flex items-center justify-center bg-base-200">
        <div className="bg-neutral text-neutral-content p-6 rounded shadow-lg w-[90%] max-w-md">
          ...
          <div className="flex justify-end gap-2">
            <button className="btn btn-error" onClick={handleLogoutAndShowForm}>
              Logout &amp; Continue
            </button>
