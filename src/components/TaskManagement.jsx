...
const TaskManagement = () => {
  ...
  const fetchTaskAnalytics = async () => {
    try {
      const res = await axios.get(
        `${BASE_URL}/api/project/${projectId}/tasks/analytics`,
        {
          withCredentials: true,
        }
      );
  ...
  const fetchComments = async (taskId) => {
    try {
      const res = await axios.get(
        `${BASE_URL}/api/project/${projectId}/task/${taskId}/comments`,
        {
          withCredentials: true,
        }
      );
        ...
        <Draggable key={task._id} draggableId={task._id} index={index}>
          {(provided) => (
            <div
              ...
              className="bg-white p-4 mb-4 rounded-lg text-[#4B4896] shadow-md hover:shadow-lg transition-all border border-gray-200"
            >
              {/* Task Title */}
              <h4 className="font-bold text-lg mb-2 text-[#4B4896]">
                {task.title}
              </h4>
              ...
              {task.dueDate && (
                <p className="text-sm font-medium mb-2">
                  🗓️ Due: 
                  <span className={`ml-1 ${dayjs(task.dueDate).diff(dayjs(), "day") <= 1 ? "text-red-600" : "text-yellow-600"}`}>
              ...
              {task.labels && task.labels.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-2">
                  {task.labels.map((label, idx) => (
                    <span key={idx} className="bg-[#10B981] text-white px-2 py-1 rounded-full text-xs">
                      {label}
                    </span>
                  ))}
                </div>
              ...
              <select
                ...
                className="w-full mb-2 p-2 bg-gray-100 border border-gray-300 text-[#4B4896] rounded-md"
              >
              ...
              {expandedTask === task._id && (
                <div className="mt-2 bg-gray-100 p-2 rounded">
                  <h5 className="text-[#4B4896] mb-2 font-bold">Comments:</h5>
                  {comments[task._id] && comments[task._id].length > 0 ? (
                    comments[task._id].map((comment) => (
                      <p key={comment._id} className="text-sm text-gray-700 mb-1">
                        <span className="font-bold text-[#4B4896]">{comment.createdBy.firstName}:</span> {comment.text}
                      </p>
                    ))
                  ) : (
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      className="w-full p-1 mb-2 rounded border border-gray-300 text-[#4B4896]"
                    />
                    <button
                      className="bg-[#10B981] p-1 rounded text-white text-sm w-full hover:bg-[#0F9A75]"
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button
                  className="bg-red-500 p-2 rounded-full text-white hover:bg-red-400 transition-colors"
                  title="Delete Task"
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="min-h-screen bg-[#8F8AC3] text-white p-6">
  {/* Back Button */}
  <button
    className="mb-4 p-2 bg-[#4B4896] rounded-full hover:bg-[#3A3778] text-white flex items-center transition-all duration-200"
    onClick={() => navigate(-1)}
  >
    <svg
  ...
  {/* Page Title */}
  <h2 className="text-3xl font-bold mb-6 text-white text-center">🗂️ Task Management</h2>

 {/* Task Management Main Layout */}
<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
  
  {/* 📝 Add/Edit Task Form */}
  <div className="bg-white p-6 rounded-xl shadow-lg text-[#4B4896]">
    <h3 className="text-lg font-bold mb-3">{isEditing ? "✏️ Edit Task" : "➕ Add Task"}</h3>
    ...
    <input
      ...
      className="w-full p-2 rounded-md border border-gray-300 bg-white text-[#4B4896] focus:ring-2 focus:ring-[#4B4896] mb-2"
    />
    ...
    {isEditing && (
      <button
        className="w-full p-2 mt-2 rounded-md bg-red-600 text-white hover:bg-red-500 transition font-semibold"
        onClick={() => {
          setIsEditing(false);
          setEditTaskId(null);
          setNewTask("");
  ...
  <div className="bg-white p-6 rounded-xl shadow-lg text-[#4B4896]">
    <h3 className="text-lg font-bold mb-3">⏳ Upcoming Deadlines</h3>
        ...
        {upcomingTasks.slice(0, 3).map((task) => {
          ...
          return (
            <li key={task._id}>
              ...
              <span className={`${daysLeft <= 1 ? "text-red-600" : "text-yellow-600"}`}>
                🗓️ {daysLeft} {daysLeft > 1 ? "days left" : "day left"}
              </span>
            </li>
          );
        })}
  ...
  {/* 📊 Task Analytics */}
  <div className="bg-white p-6 rounded-xl  shadow-lg text-[#4B4896]">
    <h3 className="text-lg font-bold mb-3">📊 Task Analytics</h3>
    <ul className="text-sm space-y-2">
      <li><strong>Total Tasks:</strong> {taskAnalytics.totalTasks}</li>
      <li><strong>Completed:</strong> {taskAnalytics.completedTasks}</li>
      <li><strong>Pending:</strong> {taskAnalytics.pendingTasks}</li>
      <li><strong>Avg Completion Time:</strong> {taskAnalytics.averageCompletionTime || "N/A"}</li>
    </ul>
  </div>
</div>
            ...
            <Droppable key={status} droppableId={status}>
              {(provided) => (
                <div
                  ...
                  className="bg-white border-2 border-[#4B4896] p-4 rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300"
                >
                  {/* Task Status Header */}
                  <h3
                  ...
                  {/* Render Tasks */}
                  <div className="space-y-3">{renderTasks(status)}</div>
