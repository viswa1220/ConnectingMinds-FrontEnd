...
const CollabChatPage = () => {
  ...
  // ---------- RENDER ----------
  return (
    <div className="bg-[#8F8AC3] text-white flex flex-col pb-4">
    {/* NAVIGATION BAR (Pinned at Top) */}
    <div className="sticky top-0 z-10 bg-[#4B4896] text-white shadow p-3 flex items-center">
      {/* Back Arrow */}
      <button onClick={() => navigate(-1)} className="text-white hover:text-gray-300 mr-4">
        <FiArrowLeft size={24} />
      </button>
      {/* Tabs (Left-Aligned) */}
      <div className="space-x-2">
        <button
          ...
          className={`px-4 py-2 rounded ${
            activeTab === "group" ? "bg-white text-[#4B4896]" : "bg-[#3A3778] text-white"
          }`}
        >
          Group Chat
        </button>
        <button
          ...
          className={`px-4 py-2 rounded ${
            activeTab === "direct" ? "bg-white text-[#4B4896]" : "bg-[#3A3778] text-white"
          }`}
        >
          Direct Chat
        </button>
    ...
    <div className="flex-1 flex justify-center mt-4">
      {/* GROUP CHAT VIEW */}
      {activeTab === "group" && (
        <div className="w-full max-w-4xl h-[70vh] bg-white text-[#4B4896] rounded-lg shadow-lg flex flex-col overflow-hidden">
          {/* Group Chat Header */}
          <div className="px-4 py-3 border-b border-gray-200 bg-[#8F8AC3] text-white">
            <h2 className="text-lg font-semibold">Group Chat</h2>
          </div>
          {/* Collaborators Row */}
          <div className="flex px-4 py-2 space-x-4 overflow-x-auto bg-gray-100">
            {collaborators.map((user) => (
              <div key={user._id} className="text-center">
                <img
                  src={user.photoUrl || "/default-avatar.png"}
                  alt={user.firstName}
                  className="w-10 h-10 rounded-full border-2 border-[#4B4896]"
                />
                <span className="block text-xs text-gray-700">{user.firstName}</span>
              </div>
            ))}
          </div>
          {/* Group Chat Messages */}
          {groupLoading ? (
            ...
            <>
                ...
                {groupMessages.map((msg, idx) => (
                  <div key={idx} className="mb-4">
                    <span className="block text-xs text-gray-500 mb-1">
                      {msg.sender === loggedInUser._id ? "You" : getSenderName(msg.sender)}
                    </span>
                    <div
                      className={`p-2 rounded-md max-w-xs ${
                        msg.sender === loggedInUser._id
                          ? "bg-[#4B4896] text-white ml-auto"
              ...
              {/* Group Message Input */}
              <div className="flex items-center p-3 bg-[#8F8AC3] border-t border-gray-300">
                ...
                <input
                  ...
                  className="flex-1 p-2 bg-white text-gray-900 rounded-l-md border border-gray-300 focus:outline-none"
                />
                <button
                  onClick={handleSendGroupMessage}
                  className="bg-[#4B4896] p-2 rounded-r-md hover:bg-[#3A3778] transition text-white ml-1"
                >
                  <FiSend size={20} />
                </button>
              </div>
      ...
      {/* DIRECT CHAT VIEW */}
      {activeTab === "direct" && (
        <div className="w-full max-w-4xl h-[70vh] bg-white text-[#4B4896] rounded-lg shadow-lg flex">
          {/* Left Sidebar: Collaborators */}
          <div className="w-1/3 border-r border-gray-300 bg-gray-100 p-4 overflow-y-auto">
            <h2 className="text-md font-bold mb-4 text-[#4B4896]">Direct Chats</h2>
                ...
                .map((user) => (
                  <li
                    ...
                    className={`p-2 rounded cursor-pointer hover:bg-gray-200 ${
                      selectedReceiverId === user._id ? "bg-gray-200" : ""
                    }`}
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <span className="text-[#4B4896]">{user.firstName} {user.lastName}</span>
                    </div>
                  </li>
                ))}
            </ul>
          ...
          <div className="flex-1 p-4">
            {loadingDirect ? (
              ...
              <p className="text-center text-gray-400">Select a collaborator to start chatting.</p>
            )}
          </div>
        </div>
      )}
